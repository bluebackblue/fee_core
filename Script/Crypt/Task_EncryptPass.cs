	...
	*/
	public class Task_EncryptPass
	{
		...
		*/
		...
		/** TaskMain
		*/
		private static async System.Threading.Tasks.Task<ResultType> TaskMain(Fee.Crypt.OnTask_CallBackInterface a_callback,byte[] a_binary,string a_pass,string a_salt,System.Threading.CancellationToken a_cancel)
		{
			ResultType t_ret;
			{
				t_ret.binary = null;
		...
		/** 実行。
		*/
		public static Fee.TaskW.Task<ResultType> Run(Fee.Crypt.OnTask_CallBackInterface a_callback,byte[] a_binary,string a_pass,string a_salt,Fee.TaskW.CancelToken a_cancel)
		{
			System.Threading.CancellationToken t_cancel_token = a_cancel.GetToken();

			return new Fee.TaskW.Task<ResultType>(() => {
				return Task_EncryptPass.TaskMain(a_callback,a_binary,a_pass,a_salt,t_cancel_token);
			});
		}
	}
}
