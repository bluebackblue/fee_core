	...
	*/
	public class Task_CreateSignaturePrivateKey
	{
		...
		*/
		...
		/** 実行。
		*/
		public static NTaskW.Task<ResultType> Run(byte[] a_binary,string a_key,NTaskW.CancelToken a_cancel)
		{
			System.Threading.CancellationToken t_cancel_token = a_cancel.GetToken();

			return new NTaskW.Task<ResultType>(() => {
				return Task_CreateSignaturePrivateKey.TaskMain(a_binary,a_key,t_cancel_token);
			});
		}
	}
}
