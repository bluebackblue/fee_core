...
{
	/** ObjectToJson_FromClass
	*/
	public class ObjectToJson_FromClass
	{
		/** Convert
		*/
			...
			try{
						...
						if(t_key_type == typeof(string)){
							...
							foreach(System.Collections.DictionaryEntry t_from_pair in t_from_dictionary){
								...
								if(t_from_listitem_key_string != null){
									...
									a_workpool.Add(new ObjectToJson_WorkPool_Item(ObjectToJson_WorkPool_Item.ModeAddAssociativeArray.Start,a_nest + 1,t_to_jsonitem,t_from_listitem_key_string,t_from_listitem_object,t_listitem_valuetype,a_from_objectoption));
								}else{
									//NULL処理。
									//keyがnullの場合は追加しない。
								}
				...
				//ICollection
				{
					...
					if(t_from_collection != null){
						...
						foreach(System.Object t_from_listitem in t_from_collection){
							a_workpool.Add(new ObjectToJson_WorkPool_Item(ObjectToJson_WorkPool_Item.ModeSetIndexArray.Start,a_nest + 1,t_to_jsonitem,t_index,t_from_listitem,t_listitem_valuetype,a_from_objectoption));
							t_index++;
						}
				...
				//IEnumerable
				{
					...
					if(t_from_enumerable != null){
						...
						foreach(System.Object t_from_listitem in t_from_enumerable){
							a_workpool.Add(new ObjectToJson_WorkPool_Item(ObjectToJson_WorkPool_Item.ModeAddIndexArray.Start,a_nest + 1,t_to_jsonitem,t_from_listitem,t_listitem_valuetype,a_from_objectoption));
						}
				...
				//class,struct
				{
					...
					foreach(System.Reflection.FieldInfo t_fieldinfo in t_fieldinfo_list){
						a_workpool.Add(new ObjectToJson_WorkPool_Item(ObjectToJson_WorkPool_Item.ModeFieldInfo.Start,a_nest + 1,t_to_jsonitem,t_fieldinfo,a_from_object));
					}
