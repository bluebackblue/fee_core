...
{
	/** ConvertSheet_FontPrefab
	*/
	#if(UNITY_EDITOR)
	public class ConvertSheet_FontPrefab
	{
		/** COMMAND
		*/
		public const string COMMAND = "<fontprefab>";
		...
		*/
		public class ListItem
		{
			/** fontprefab_command
			*/
			public string fontprefab_command;

			/** fontprefab_tag
			*/
			public string fontprefab_tag;

			/** font_assetspath
			*/
			public string fontprefab_assetspath;
		}
		...
		public static void Convert(string a_convert_param,Fee.File.Path a_assets_path,Fee.JsonItem.JsonItem[] a_sheet,Fee.JsonSheet.ConvertParam a_convertparam)
		{
			try{
				if(a_sheet != null){
						...
						if(a_sheet[ii] != null){
							...
							if(t_sheet != null){
								for(int jj=0;jj<t_sheet.Count;jj++){
									if(ConvertSheet_FontPrefab.COMMAND_ITEM == t_sheet[jj].fontprefab_command){
										//<item>

										t_tag_list.Add(t_sheet[jj].fontprefab_tag);
										t_path_list.Add(new File.Path(t_sheet[jj].fontprefab_assetspath));
									}else{
										//無関係。複合シート。
									}
								}
					...
					//保存。
					{
						Fee.Instantiate.FontList_Tool.ResourceItem[] t_resource_list = new Instantiate.FontList_Tool.ResourceItem[t_tag_list.Count];
						for(int ii=0;ii<t_tag_list.Count;ii++){
							t_resource_list[ii] = new Instantiate.FontList_Tool.ResourceItem(t_tag_list[ii],t_path_list[ii]);
						}

						UnityEngine.GameObject t_prefab = new UnityEngine.GameObject("prefab_temp");
						Fee.Instantiate.FontList_Tool.Add(t_prefab,t_resource_list);
